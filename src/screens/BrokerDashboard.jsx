...
export default function BrokerDashboard({ state: appState, setScreen }) {
  ...
  const inp = {width:"100%",background:"#1a1a1a",border:"1px solid #333",borderRadius:"8px",padding:"10px 12px",fontSize:"13px",color:"rgba(255,255,255,0.65)",fontFamily:"inherit",outline:"none",WebkitTextFillColor:"rgba(255,255,255,0.65)"}
  ...
  return (
    <div style={{minHeight:"100vh",background:G.bg}}>
      ...
      <div style={{maxWidth:"980px",margin:"0 auto",padding:"40px 28px 100px"}}>
        ...
        {/* Seat stats */}
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1px",background:G.border,borderRadius:"12px",overflow:"hidden",marginBottom:"40px",border:"1px solid rgba(255,255,255,0.25)"}}>
        ...
        {/* Team card */}
        <div style={{background:G.card,border:"1px solid rgba(255,255,255,0.25)",borderRadius:"14px",overflow:"hidden",marginBottom:"32px"}}>
          {/* Add agent */}
            ...
            {agents.length===0 ? (
              ...
              <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px 28px",borderBottom:i<agents.length-1?`1px solid ${G.border}`:"none"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
                  <div style={{background:agent.status==="active"?G.aquaDim:"rgba(255,255,255,0.05)",border:`1px solid ${agent.status==="active"?G.aquaBorder:G.border}`,borderRadius:"100px",padding:"3px 10px",fontSize:"10px",fontWeight:"700",color:agent.status==="active"?G.aqua:G.muted}}>{(agent.status==="active"?"ACTIVE":"PENDING").toUpperCase()}</div>
                  <button onClick={()=>removeAgent(agent,i)} style={{background:"transparent",color:"rgba(255,80,80,0.7)",border:"1px solid rgba(255,80,80,0.2)",borderRadius:"6px",padding:"4px 10px",fontSize:"11px",cursor:"pointer",fontFamily:"inherit"}}>Remove</button>
                </div>
              </div>
            ))}
          </div>
