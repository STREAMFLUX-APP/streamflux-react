...
import { Chips as ChipsOriginal } from '../components/shared/Chips.jsx'
        ...
        {options.map(opt=>{
          ...
          return(
            <button key={val} onClick={()=>onToggle(val)} style={{
              background:active?'rgba(42,184,212,0.1)':'#060608',
              border:'1px solid '+(active?'#2AB8D4':'rgba(42,184,212,0.18)'),
              borderRadius:'8px',color:active?'#2AB8D4':'rgba(255,255,255,0.65)',
              fontSize:'12px',padding:sub?'10px 14px':'7px 12px',
              cursor:'pointer',fontFamily:'inherit',textAlign:'left',
              WebkitTapHighlightColor:'rgba(42,184,212,0.3)',
              touchAction:'manipulation',
            }}>
              <div style={{fontWeight:active?'700':'400'}}>{lbl}</div>
              {sub&&<div style={{fontSize:'11px',color:active?'rgba(42,184,212,0.7)':'rgba(255,255,255,0.5)',marginTop:'2px'}}>{sub}</div>}
            </button>
          )
        })}
      </div>
